...
Write-Host " EduVigIA 2.0.0-F6-R1 - Homologacao F6" -ForegroundColor Cyan
Write-Host " Mapas Operacionais + F5 Video Wall preservado" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
try {
    ...
    if ($Version -eq "2.0.0-F6-R1") { Pass "VERSION" } else { Fail "VERSION" $Version }
    ...
    try {
        ...
        if ($Head -match '20260910_206_f5') { Pass "ALEMBIC_F5_HEAD" } else { Fail "ALEMBIC_F5_HEAD" $Head }
    } catch { Fail "ALEMBIC_F5_HEAD" $_.Exception.Message }
    ...
    $TestDbUrl = "sqlite:////tmp/eduvigia-f6-test.db"
...
if ($Failed -gt 0) {
    Write-Host ("EDUVIGIA_F6_OPERATIONAL_MAPS_CORE=REJECTED|FAILURES={0}" -f $Failed) -ForegroundColor Red
    exit 1
}
