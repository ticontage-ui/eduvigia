...
Write-Host " EduVigIA 2.0.0-F3-R2 - Homologacao F3-R2" -ForegroundColor Cyan
Write-Host " Multi-Channel / Multi-Sensor + PTZ preservado" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
try {
    ...
    try {
        ...
        if ($Head -match '20260909_204_f3r2') { Pass "ALEMBIC_F3R2_HEAD" } else { Fail "ALEMBIC_F3R2_HEAD" $Head }
    } catch { Fail "ALEMBIC_F3R2_HEAD" $_.Exception.Message }
    ...
    $TestDbUrl = "sqlite:////tmp/eduvigia-f3-r2-test.db"
    ...
    if ($TestApiImage) {
        $ImportResult = Invoke-DockerCapture -Arguments @(
            ...
            "-w", "/app",
            $TestApiImage,
            "python", "-c", "import app; import app.application; print('EDUVIGIA_F3R2_TEST_RUNNER_IMPORT_OK')"
        )
        $ImportProbe = ($ImportResult.Output -join "`n")
        if ($ImportResult.ExitCode -eq 0 -and $ImportProbe -match 'EDUVIGIA_F3R2_TEST_RUNNER_IMPORT_OK') { Pass "TEST_RUNNER_APP_IMPORT" }
...
if ($Failed -gt 0) {
    Write-Host ("EDUVIGIA_F3R2_MULTICHANNEL_CORE=REJECTED|FAILURES={0}" -f $Failed) -ForegroundColor Red
    exit 1
}
