...
Write-Host " EduVigIA 2.0.0-F7-R1 - Homologacao F7" -ForegroundColor Cyan
Write-Host " Plantas Baixas + F6 Mapas Operacionais preservado" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
try {
    ...
    if ($Version -eq "2.0.0-F7-R1") { Pass "VERSION" } else { Fail "VERSION" $Version }
    ...
    try {
        ...
        $ApiRequired = @("DATABASE_URL", "REDIS_URL", "EDUVIGIA_BOOTSTRAP_ADMIN_PASSWORD", "EDUVIGIA_CREDENTIAL_KEY", "EDUVIGIA_STREAM_SIGNING_KEY", "EDUVIGIA_PTZ_LEASE_SECONDS", "CORS_ORIGINS", "EDUVIGIA_MAP_TILE_URL", "EDUVIGIA_MAP_ATTRIBUTION")
    ...
    try {
        ...
        if ($Head -match '20260910_207_f7') { Pass "ALEMBIC_F7_HEAD" } else { Fail "ALEMBIC_F7_HEAD" $Head }
    } catch { Fail "ALEMBIC_F7_HEAD" $_.Exception.Message }

    try {
        # Verifier R2: legacy DB contracts validate schema/capabilities only.
        # Current lineage/head is validated once by ALEMBIC_F7_HEAD above.
        $LegacyPinnedHead = "20260910_" + "206_f5"
        $VerifierRaw = Get-Content $PSCommandPath -Raw
        $LegacyPinPattern = [regex]::Escape("v=='$LegacyPinnedHead'")
        if ($VerifierRaw -notmatch $LegacyPinPattern) { Pass "LEGACY_DB_CONTRACT_VERSION_AGNOSTIC" }
        else { Fail "LEGACY_DB_CONTRACT_VERSION_AGNOSTIC" "contrato legado ainda exige head F5" }
    } catch { Fail "LEGACY_DB_CONTRACT_VERSION_AGNOSTIC" $_.Exception.Message }
    ...
    try {
        $F5Preflight = (& docker compose exec -T api python -m app.vms_f5_preflight) -join ""
        if ($F5Preflight -match 'EDUVIGIA_F5_VIDEO_WALL_PREFLIGHT_OK') { Pass "BACKEND_F5_VIDEO_WALL_CONTRACT" }
        else { Fail "BACKEND_F5_VIDEO_WALL_CONTRACT" $F5Preflight }
    } catch { Fail "BACKEND_F5_VIDEO_WALL_CONTRACT" $_.Exception.Message }

    try {
        $F5Db = (& docker compose exec -T api python -c "import os; from sqlalchemy import create_engine,inspect,text; e=create_engine(os.environ['DATABASE_URL']); i=inspect(e); layouts=i.has_table('video_wall_layouts'); slots=i.has_table('video_wall_slots'); lcols={c['name'] for c in i.get_columns('video_wall_layouts')} if layouts else set(); scols={c['name'] for c in i.get_columns('video_wall_slots')} if slots else set(); c=e.connect(); v=c.execute(text('select version_num from alembic_version')).scalar(); c.close(); req_l={'user_id','name','grid_size','quality_mode','is_default'}; req_s={'layout_id','slot_index','camera_id'}; print('OK' if layouts and slots and req_l.issubset(lcols) and req_s.issubset(scols) else 'FAIL:'+repr({'layouts':layouts,'slots':slots,'missing_layout':sorted(req_l-lcols),'missing_slot':sorted(req_s-scols),'version':v}))") -join ""
        if ($F5Db.Trim() -eq 'OK') { Pass "DATABASE_F5_VIDEO_WALL_CONTRACT" }
        else { Fail "DATABASE_F5_VIDEO_WALL_CONTRACT" $F5Db }
    } catch { Fail "DATABASE_F5_VIDEO_WALL_CONTRACT" $_.Exception.Message }

    try {
        $F5Security = (& docker compose exec -T api python -c "from app.application import role_permissions; writers=['GESTOR_SECRETARIA','SUPERVISOR_GUARDA','OPERADOR_GUARDA','DESPACHANTE_GUARDA']; ok=all('wall:view' in role_permissions(r,None) and 'wall:write' in role_permissions(r,None) for r in writers) and 'wall:view' in role_permissions('TECNICO',None) and 'wall:write' not in role_permissions('GESTOR_ESCOLA',1); print('OK' if ok else 'FAIL')") -join ""
        if ($F5Security.Trim() -eq 'OK') { Pass "F5_VIDEO_WALL_RBAC_CONTRACT" }
        else { Fail "F5_VIDEO_WALL_RBAC_CONTRACT" $F5Security }
    } catch { Fail "F5_VIDEO_WALL_RBAC_CONTRACT" $_.Exception.Message }

    try {
        $F5App = Get-Content (Join-Path $ProjectPath "frontend\src\app\App.jsx") -Raw
        $F5Required = @(
            'Video Wall Operacional',
            '/video-wall/layouts',
            'wall:view',
            'wall:write',
            'Tela cheia',
        ...
        $F5Missing = @($F5Required | Where-Object { $F5App -notmatch [regex]::Escape($_) })
        if ($F5Missing.Count -eq 0) { Pass "FRONTEND_F5_VIDEO_WALL_CONTRACT" }
        else { Fail "FRONTEND_F5_VIDEO_WALL_CONTRACT" ("missing={0}" -f ($F5Missing -join ',')) }
    } catch { Fail "FRONTEND_F5_VIDEO_WALL_CONTRACT" $_.Exception.Message }

    try {
        $F5Application = Get-Content (Join-Path $ProjectPath "backend\app\application.py") -Raw
        $OwnerScoped = (
            $F5Application -match 'VideoWallLayout.user_id == user.id' -and
            $F5Application -match 'row.user_id != user.id' -and
            $F5Application -match 'A mesma câmera não pode ocupar dois slots'
        )
        if ($OwnerScoped) { Pass "F5_LAYOUT_OWNER_AND_DUPLICATE_GUARD" }
        else { Fail "F5_LAYOUT_OWNER_AND_DUPLICATE_GUARD" "layout deve ser isolado por operador e impedir câmera duplicada" }
    } catch { Fail "F5_LAYOUT_OWNER_AND_DUPLICATE_GUARD" $_.Exception.Message }


    try {
        $F6Preflight = (& docker compose exec -T api python -m app.vms_f6_preflight) -join ""
        if ($F6Preflight -match 'EDUVIGIA_F6_OPERATIONAL_MAPS_PREFLIGHT_OK') { Pass "BACKEND_F6_OPERATIONAL_MAPS_CONTRACT" }
        else { Fail "BACKEND_F6_OPERATIONAL_MAPS_CONTRACT" $F6Preflight }
    } catch { Fail "BACKEND_F6_OPERATIONAL_MAPS_CONTRACT" $_.Exception.Message }

    try {
        $F6Db = (& docker compose exec -T api python -c "import os; from sqlalchemy import create_engine,inspect,text; e=create_engine(os.environ['DATABASE_URL']); i=inspect(e); cols={c['name'] for c in i.get_columns('schools')}; c=e.connect(); v=c.execute(text('select version_num from alembic_version')).scalar(); c.close(); req={'latitude','longitude','operational_status'}; print('OK' if req.issubset(cols) else 'FAIL:'+repr({'missing':sorted(req-cols),'version':v}))") -join ""
        if ($F6Db.Trim() -eq 'OK') { Pass "DATABASE_F6_MAP_COORDINATE_CONTRACT"; Pass "ALEMBIC_F6_SCHEMA_BASE" }
        else { Fail "DATABASE_F6_MAP_COORDINATE_CONTRACT" $F6Db }
    } catch { Fail "DATABASE_F6_MAP_COORDINATE_CONTRACT" $_.Exception.Message }

    try {
        $F6Security = (& docker compose exec -T api python -c "from app.application import role_permissions; roles=[('GESTOR_SECRETARIA',None),('SUPERVISOR_GUARDA',None),('OPERADOR_GUARDA',None),('DESPACHANTE_GUARDA',None),('GESTOR_ESCOLA',1),('OPERADOR_ESCOLA',1),('TECNICO',None)]; ok=all('maps:view' in role_permissions(r,s) for r,s in roles); print('OK' if ok else 'FAIL')") -join ""
        if ($F6Security.Trim() -eq 'OK') { Pass "F6_MAP_RBAC_SCOPE_CONTRACT" }
        else { Fail "F6_MAP_RBAC_SCOPE_CONTRACT" $F6Security }
    } catch { Fail "F6_MAP_RBAC_SCOPE_CONTRACT" $_.Exception.Message }

    try {
        $F6App = Get-Content (Join-Path $ProjectPath "frontend\src\app\App.jsx") -Raw
        $F6Required = @(
            'Mapa Operacional',
            '/maps/overview',
            'maps:view',
        ...
        $F6Missing = @($F6Required | Where-Object { $F6App -notmatch [regex]::Escape($_) })
        if ($F6Missing.Count -eq 0) { Pass "FRONTEND_F6_OPERATIONAL_MAP_CONTRACT" }
        else { Fail "FRONTEND_F6_OPERATIONAL_MAP_CONTRACT" ("missing={0}" -f ($F6Missing -join ',')) }
    } catch { Fail "FRONTEND_F6_OPERATIONAL_MAP_CONTRACT" $_.Exception.Message }

    try {
        $F6Application = Get-Content (Join-Path $ProjectPath "backend\app\application.py") -Raw
        $ComposeDev = Get-Content (Join-Path $ProjectPath "docker-compose.yml") -Raw
        $ComposeProd = Get-Content (Join-Path $ProjectPath "docker-compose.prod.yml") -Raw
        $Contract = (
            $F6Application -match '@app.get\("/maps/overview"\)' -and
            $F6Application -match 'scoped_school_id\(user\)' -and
            $F6Application -match 'EDUVIGIA_MAP_TILE_URL' -and
            $ComposeDev -match 'EDUVIGIA_MAP_TILE_URL' -and
            $ComposeProd -match 'EDUVIGIA_MAP_TILE_URL'
        )
        if ($Contract) { Pass "F6_ONPREM_TILE_CONFIG_CONTRACT" }
        else { Fail "F6_ONPREM_TILE_CONFIG_CONTRACT" "mapa deve respeitar escopo escolar e tile server configuravel" }
    } catch { Fail "F6_ONPREM_TILE_CONFIG_CONTRACT" $_.Exception.Message }

    try {
        $F7Preflight = (& docker compose exec -T api python -m app.vms_f7_preflight) -join ""
        if ($F7Preflight -match 'EDUVIGIA_F7_FLOOR_PLANS_PREFLIGHT_OK') { Pass "BACKEND_F7_FLOOR_PLANS_CONTRACT" }
        else { Fail "BACKEND_F7_FLOOR_PLANS_CONTRACT" $F7Preflight }
    } catch { Fail "BACKEND_F7_FLOOR_PLANS_CONTRACT" $_.Exception.Message }

    try {
        $F7Db = (& docker compose exec -T api python -c "import os; from sqlalchemy import create_engine,inspect,text; e=create_engine(os.environ['DATABASE_URL']); i=inspect(e); tables=set(i.get_table_names()); c=e.connect(); v=c.execute(text('select version_num from alembic_version')).scalar(); c.close(); cols={x['name'] for x in i.get_columns('floor_plan_cameras')} if 'floor_plan_cameras' in tables else set(); req={'floor_plans','floor_plan_cameras'}; need={'floor_plan_id','camera_id','x_percent','y_percent','rotation_deg'}; print('OK' if req.issubset(tables) and need.issubset(cols) and v=='20260910_207_f7' else 'FAIL:'+repr({'tables':sorted(req-tables),'cols':sorted(need-cols),'version':v}))") -join ""
        if ($F7Db.Trim() -eq 'OK') { Pass "DATABASE_F7_FLOOR_PLANS_CONTRACT" }
        else { Fail "DATABASE_F7_FLOOR_PLANS_CONTRACT" $F7Db }
    } catch { Fail "DATABASE_F7_FLOOR_PLANS_CONTRACT" $_.Exception.Message }

    try {
        $F7Security = (& docker compose exec -T api python -c "from app.application import role_permissions; viewers=[('GESTOR_SECRETARIA',None),('SUPERVISOR_GUARDA',None),('OPERADOR_GUARDA',None),('DESPACHANTE_GUARDA',None),('GESTOR_ESCOLA',1),('OPERADOR_ESCOLA',1),('TECNICO',None)]; writers=[('GESTOR_SECRETARIA',None),('GESTOR_ESCOLA',1),('TECNICO',None)]; blocked=[('SUPERVISOR_GUARDA',None),('OPERADOR_GUARDA',None),('DESPACHANTE_GUARDA',None),('OPERADOR_ESCOLA',1)]; ok=all('floorplans:view' in role_permissions(r,s) for r,s in viewers) and all('floorplans:write' in role_permissions(r,s) for r,s in writers) and all('floorplans:write' not in role_permissions(r,s) for r,s in blocked); print('OK' if ok else 'FAIL')") -join ""
        if ($F7Security.Trim() -eq 'OK') { Pass "F7_FLOOR_PLANS_RBAC_SCOPE_CONTRACT" }
        else { Fail "F7_FLOOR_PLANS_RBAC_SCOPE_CONTRACT" $F7Security }
    } catch { Fail "F7_FLOOR_PLANS_RBAC_SCOPE_CONTRACT" $_.Exception.Message }

    try {
        $F7App = Get-Content (Join-Path $ProjectPath "frontend\src\app\App.jsx") -Raw
        $F7Required = @('Plantas Baixas','/floor-plans','floorplans:view','floorplans:write','FloorPlansPage','Imagem da planta','Câmera/sensor para posicionar','rotation_deg','logical_channel','sensor_type')
        $F7Missing = @($F7Required | Where-Object { $F7App -notmatch [regex]::Escape($_) })
        if ($F7Missing.Count -eq 0) { Pass "FRONTEND_F7_FLOOR_PLANS_CONTRACT" }
        else { Fail "FRONTEND_F7_FLOOR_PLANS_CONTRACT" ("missing={0}" -f ($F7Missing -join ',')) }
    } catch { Fail "FRONTEND_F7_FLOOR_PLANS_CONTRACT" $_.Exception.Message }

    try {
        $F7Application = Get-Content (Join-Path $ProjectPath "backend\app\application.py") -Raw
        $Safe = (
            $F7Application -match 'image/jpeg' -and $F7Application -match 'image/png' -and $F7Application -match 'image/webp' -and
            $F7Application -notmatch 'image/svg\+xml' -and $F7Application -match '15 \* 1024 \* 1024' -and
            $F7Application -match 'ensure_school_access\(user, school_id\)' -and
            $F7Application -match 'A mesma câmera não pode ser posicionada duas vezes na mesma planta' -and
            $F7Application -match 'A câmera deve pertencer à mesma escola da planta'
        )
        if ($Safe) { Pass "F7_FLOOR_PLAN_FILE_AND_SCOPE_SECURITY" }
        else { Fail "F7_FLOOR_PLAN_FILE_AND_SCOPE_SECURITY" "upload/escopo de planta incompleto" }
    } catch { Fail "F7_FLOOR_PLAN_FILE_AND_SCOPE_SECURITY" $_.Exception.Message }

    $TestDbUrl = "sqlite:////tmp/eduvigia-f7-test.db"
    ...
    if ($TestApiImage) {
        $ImportResult = Invoke-DockerCapture -Arguments @(
            ...
            "-w", "/app",
            $TestApiImage,
            "python", "-c", "import app; import app.application; print('EDUVIGIA_F6_TEST_RUNNER_IMPORT_OK')"
        )
        $ImportProbe = ($ImportResult.Output -join "`n")
        if ($ImportResult.ExitCode -eq 0 -and $ImportProbe -match 'EDUVIGIA_F6_TEST_RUNNER_IMPORT_OK') { Pass "TEST_RUNNER_APP_IMPORT" }
...
if ($Failed -gt 0) {
    Write-Host ("EDUVIGIA_F7_FLOOR_PLANS_CORE=REJECTED|FAILURES={0}" -f $Failed) -ForegroundColor Red
    exit 1
}
...
Write-Host "EDUVIGIA_F5_VIDEO_WALL_CORE=APPROVED" -ForegroundColor Green
Write-Host "EDUVIGIA_F5_VIDEO_WALL_DISPLAY=PENDING_FIELD_TEST" -ForegroundColor Yellow
Write-Host "EDUVIGIA_F6_OPERATIONAL_MAPS_CORE=APPROVED" -ForegroundColor Green
Write-Host "EDUVIGIA_F6_MAP_TILES=CONFIGURABLE_ONPREM" -ForegroundColor Green
Write-Host "EDUVIGIA_F6_MAP_DATA=PENDING_FIELD_COORDINATES" -ForegroundColor Yellow
Write-Host "EDUVIGIA_F7_FLOOR_PLANS_CORE=APPROVED" -ForegroundColor Green
Write-Host "EDUVIGIA_F7_FLOOR_PLANS_FIELD=PENDING_FIELD_PLAN" -ForegroundColor Yellow
