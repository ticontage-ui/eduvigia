...
Write-Host " EduVigIA 2.0.0-F5-R1 - Homologacao F5" -ForegroundColor Cyan
Write-Host " Video Wall Operacional + F4 Playback preservado" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
try {
    ...
    if ($Version -eq "2.0.0-F5-R1") { Pass "VERSION" } else { Fail "VERSION" $Version }
    ...
    $TestDbUrl = "sqlite:////tmp/eduvigia-f5-test.db"
    ...
    if ($TestApiImage) {
        $ImportResult = Invoke-DockerCapture -Arguments @(
            ...
            "-w", "/app",
            $TestApiImage,
            "python", "-c", "import app; import app.application; print('EDUVIGIA_F5_TEST_RUNNER_IMPORT_OK')"
        )
        $ImportProbe = ($ImportResult.Output -join "`n")
        if ($ImportResult.ExitCode -eq 0 -and $ImportProbe -match 'EDUVIGIA_F5_TEST_RUNNER_IMPORT_OK') { Pass "TEST_RUNNER_APP_IMPORT" }
...
if ($Failed -gt 0) {
    Write-Host ("EDUVIGIA_F5_VIDEO_WALL_CORE=REJECTED|FAILURES={0}" -f $Failed) -ForegroundColor Red
    exit 1
}
